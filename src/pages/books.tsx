              ...
              <SiteTitle>
                {props.data.logo ? (
                  <img
                    ...
                    alt={config.titleBooks}
                  />
                ) : (
                  config.titleBooks
                )}
              </SiteTitle>
              <SiteDescription>{config.descriptionBooks}</SiteDescription>
            </SiteHeaderContent>
            <SiteNav isHome />
          </div>
        </header>
...
export const pageQuery = graphql`
  query {
    logo: file(relativePath: { eq: "img/mehran-logo.png" }) {
      childImageSharp {
    ...
    header: file(relativePath: { eq: "img/books-cover.png" }) {
      childImageSharp {
        # Specify the image processing specifications right in the query.
        # Makes it trivial to update as your page's design changes.
        fluid(maxWidth: 2000) {
    ...
    allMarkdownRemark(
      ...
      filter: { frontmatter: { draft: { ne: true }, layout: { eq: "book"} } },
      limit: 1000,
    ) {
      edges {
        node {
